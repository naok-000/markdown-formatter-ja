...
#[test]
fn accepts_escape_policy_option() {
    let mut child = binary()
        .args(["--escape-policy", "minimal"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
    ...
    child
        ...
        .as_mut()
        .unwrap()
        .write_all("foo_bar > quote".as_bytes())
        .unwrap();
...
#[test]
fn rejects_invalid_escape_policy() {
    let output = binary()
        .args(["--escape-policy", "unknown"])
        .output()
        .unwrap();
...
fn prints_help() {
    ...
    assert!(stdout.contains("--escape-policy"));
